...
void userspace_elf_loader_task(void) {
    ...
    /* -8: System V ABI требует rsp % 16 == 8 на входе в функцию (как после
     * call). GCC с -O2 кладёт movaps/movdqa на стек, рассчитывая на это.
     * С ровно выровненным rsp (0x804000) все «выровненные» адреса смещены
     * на 8 → movaps по невыровненному адресу → #GP(0) в ring 3. */
    uint64_t user_stack_top = 0x800000 + 16384 - 8;
