...
int virtio_gpu_init(void) {
    ...
    cr->resource_id = RES_ID; cr->format = VIRTIO_GPU_FORMAT_B8G8R8A8_UNORM;
    cr->width = g_w; cr->height = g_h;
    if (gpu_cmd(sizeof(*cr), sizeof(gpu_hdr_t), VIRTIO_GPU_RESP_OK_NODATA) != 0) {
        fb_puts("[virtio-gpu] RESOURCE_CREATE_2D fail\n"); return 0;
    }

    /* 9. выделить fb-backing (виртуально непрерывный, page-aligned) */
    ...
    /* собрать mem_entry по страницам fb в pmm-страницы */
    gpu_mem_entry_t *ent = 0; uint32_t per_page = 0x1000 / sizeof(gpu_mem_entry_t);
    for (uint32_t i = 0; i < g_entry_page_count; i++) {
        g_entry_pages[i] = pmm_alloc_zero();
        if (!g_entry_pages[i]) return 0;
    }
    for (uint64_t pg = 0; pg < g_fb_pages; pg++) {
    ...
    /* 10. RESOURCE_ATTACH_BACKING (hdr-страница + entry-страницы) */
    ...
    ab->resource_id = RES_ID; ab->nr_entries = (uint32_t)g_fb_pages;
    {
        vq_seg_t s[2 + MAX_ENTRY_PAGES]; int n = 0;
        s[n++] = (vq_seg_t){ cmd_phys, sizeof(*ab), 0 };
        uint64_t remain = g_fb_pages * sizeof(gpu_mem_entry_t);
        for (uint32_t i = 0; i < g_entry_page_count; i++) {
    ...
    sc->r.x = 0; sc->r.y = 0; sc->r.width = g_w; sc->r.height = g_h;
    sc->scanout_id = 0; sc->resource_id = RES_ID;
