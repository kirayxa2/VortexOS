...
static void pit_handler(interrupt_frame_t *frame) {
     ...
     * (fix #1): здесь, в обработчике прерывания, мы НИЧЕГО не рисуем — лишь
     * ставим флаг. Реальную отрисовку делает задача "render" (wm_render_task)
     * с включёнными прерываниями. Сколько бы пакетов ни сыпала мышь,
     * recomposite случится максимум 1 раз/тик, а не на каждый PS/2-пакет.
     *
     * Флаг ставим КАЖДЫЙ тик (=> до 100 FPS при 100 Hz, было 50): задача
     * рендера всё равно рисует только когда что-то изменилось (g_needs_redraw/
     * g_cursor_moved/dirty-флаги), так что в простое это бесплатно, а движение
     * курсора и перетаскивание окон становятся заметно плавнее. */
    render_request = 1;
